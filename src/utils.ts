...
export async function sendVoicemailEmail(params: { from: string, recordingUrl: string, timestamp: string, audioBuffer: Buffer }): Promise<void> {
...
export async function sendSmsNotification(params: { from: string, timestamp: string, summary?: string }): Promise<void> {
    const body = params.summary
        ? `New enquiry from ${params.from}.\n${params.summary}`
        : `New enquiry from ${params.from}. Voicemail received at ${params.timestamp}.`
